...
addreses will be our controlled addresses.

Each `minter` has a `mintingAllowance`, which we configure. The
`mintingAllowance` is how many tokens that minter may issue, and as a `minter`
issues tokens, its `mintingAllowance` declines. We will periodically reset
...
Can add minters via the `configureMinter` method. When a minter is
...
continue minting. When a `minter`'s allowance is low, you can make another
call to `configureMinter` to reset the `mintingAllowance` to a higher value.

### Removing Minters

Can remove minters via the `removeMinter` method. This will remove the
...
Can blacklist an address via the `blacklist` method. The specified `account`
...
Can remove an address from the blacklist via the `unblacklist` method. The
...
that caused us to pause the contract.

### Pause

Can pause the contract via the `pause` method. This method will set the
paused flag to true.
...
Can unpause the contract via the `unpause` method. This method will set
...
calls to `FiatToken` via delegatecall. This pattern allows us to upgrade the
logic of any deployed tokens seamlessly.

- Can upgrade the token via a call to `upgradeTo` or `upgradeToAndCall`
